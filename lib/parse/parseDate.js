...
function parseDate(value) {
  ...
  const month = ref ? getMonthName(ref.toLowerCase()) : ref

  // Make sure if february have correct day value
  if (month === "february") {
    if (isLeap(year) && day > 29) {
      throw new RangeError(
        `Incorrect day for february has been set: ${day}. ` +
        "This value must not be more than 29 in a leap year."
      )
    } else if (day > 28) {
      throw new RangeError(
        `Incorrect day for february has been set: ${day}. ` +
        "This value must not be more than 28 in a non-leap year."
      )
    }
  }
